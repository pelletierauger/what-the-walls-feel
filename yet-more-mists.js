...
void main() {
    ...
    vec3 col = CircleRGB(uv, p2, 1.9, 0.2, vec3(0.5, 0.4, 0.0));
...
`;
    ...
    // ------------------------------------------------------------------------------//
    // Interesting black swirl that will not be used but that I like anyway.
    // mustard-like and blending itself with blendyCavern.display()
    // using the mix() function.
    // ------------------------------------------------------------------------------//
    mistyProgram2.fragText = `
...
void main() {
    ...
    float t = time * 0.0025;
    vec2 p = vec2(10.0, 1.004);
    vec2 p2 = vec2(0.405, 0.22);
    vec3 col = CircleRGB(uv, p2, 1.2, 0.005, vec3(0.005, 0.5, 0.25) * 4.0);
    // col.r *= sin(length(uv - p2) * 2.);
//     col.g *= sin(length(uv - p) * (6. + sin(1. * 20.) * 0.01));
    vec2 newV = (uv + tan(uv.y * uv.x * 40.)) - (p2 * 0.1 + sin(uv.x * 23. + uv.y * 10.));
//     newV += cos(sin(uv * 15.));
    newV += cos(sin((uv * vec2(4.0, 4.0)) * 15.0 * 2.0));
    col *= sin(atan(uv.y * 12.1, newV.x * 40.));
        // col *= cos(atan(uv.x * 12.1, newV.x * 4.));
//     col *= sin(sin(atan(newV.x, newV.y) * (sin(t * 100.) * 1.2)) * 1. + sin(t * 10.) * 10.);
    float rando = rand(uv * 101.001) * 0.25 * 0.25 * 0.75;
// //     col.r -= rando * 1.;
//      col.g *= 0.5;
...
//     gl_FragColor.r *= 1.5;
    ...
    // gl_FragColor.a = 0.5;
    // wilder.rgb = wilder.bgr * 0.76;
    gl_FragColor.rgb = mix(gl_FragColor.rgb, wilder, 0.);
    // gl_FragColor.b = mix(gl_FragColor.b, min(max(gl_FragColor.g, gl_FragColor.r * 0.2 - rando), 1.0), 0.2);
//     gl_FragColor.rgb = GammaCorrection(LevelsControlInputRange(gl_FragColor.rgb, 0.1, 0.9), 1.0);
//     gl_FragColor.rgb = pow(gl_FragColor.rgb, 1.0 / vec3(1.2));
    // gl_FragColor.rgb = hueShift2(gl_FragColor.rgb, PI * 0.5);
    gl_FragColor.rgb = LevelsControlInput(gl_FragColor.rgb, 0.05, vec3(1.0), 0.95);
