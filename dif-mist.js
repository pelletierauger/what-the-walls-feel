...
//     gl_FragColor.r *= 1.5;
    wilder.g += 1. * (0.45 - uv.y) * 1. * wilder.g;
//     wilder.r -= 0.5 * (0.45 + uv.y) * 1. * wilder.g;
    wilder.g *= 1.4;
    wilder.b *= 4.0;
    gl_FragColor.a *= alpha;
    // gl_FragColor = vec4(1.0, 0.0, 0.0, 1.0);
...
//     hsv.r += 0.5;
    ...
    gl_FragColor.b += min(sin(abs(col.r + uv.x * 5.) * 0.125 * time) * 1.5, gl_FragColor.r);
//     gl_FragColor.rgb = hsv2rgb(hsv);
}
// endGLSL
`;
...
for (let i = 0; i < scenes.length; i++) {
    if (scenes[i].name !== "overture" && scenes[i].name !== "credits") {
        scenes[i].display = deepSeaDisplay;
    }
}
escherLikeRiver.display = deepSeaDisplay;
escherLikeRiver2.display = deepSeaDisplay;
...
// for (let i = 0; i < scenes.length; i++) {
//     if (scenes[i].name !== "overture" && scenes[i].name !== "credits") {
//         scenes[i].display = mistyDisplay2;
//     }
// }

// //--------------------------------------------------------------------

// escherLikeRiver.display = mistyDisplay2;
// escherLikeRiver2.display = mistyDisplay2;
// escherLikeStaircases.display = mistyDisplay2;
// plantsSofter.display = mistyDisplay2;
// middleOfTheMuscularDistanceField.display = mistyDisplay2;
// muscleFibersRebuildingThemselves.display = mistyDisplay2;
